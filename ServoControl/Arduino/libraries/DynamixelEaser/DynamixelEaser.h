...
#ifndef DynamixelEaser_h
#define DynamixelEaser_h
...
#include "../Dynamixel_Serial/Dynamixel_Serial.h"
...
class DynamixelEaser 
{
private:
    //Dynamixel dynamixel;
    int servo;      // what servo we're operating on
...
public:
    ...
    void init();
    void init(int baud);
    void init(int baud, int controlPin);
    // set up a servoeaser to use a particular servo
    //void begin(Servo s, int frameTime, int startPos);
    void begin(int servoID, int frameTime); //, int startPos);
    void begin(int servoID, int frameTime, ServoMove* moves, int movesCount);
